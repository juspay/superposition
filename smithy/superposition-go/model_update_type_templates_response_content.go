/*
Superposition

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2025-03-05
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package superposition_sdk_go

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the UpdateTypeTemplatesResponseContent type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateTypeTemplatesResponseContent{}

// UpdateTypeTemplatesResponseContent struct for UpdateTypeTemplatesResponseContent
type UpdateTypeTemplatesResponseContent struct {
	TypeName string `json:"type_name"`
	TypeSchema interface{} `json:"type_schema"`
	Description string `json:"description"`
	ChangeReason string `json:"change_reason"`
	CreatedBy string `json:"created_by"`
	CreatedAt time.Time `json:"created_at"`
	LastModifiedAt time.Time `json:"last_modified_at"`
	LastModifiedBy string `json:"last_modified_by"`
}

type _UpdateTypeTemplatesResponseContent UpdateTypeTemplatesResponseContent

// NewUpdateTypeTemplatesResponseContent instantiates a new UpdateTypeTemplatesResponseContent object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateTypeTemplatesResponseContent(typeName string, typeSchema interface{}, description string, changeReason string, createdBy string, createdAt time.Time, lastModifiedAt time.Time, lastModifiedBy string) *UpdateTypeTemplatesResponseContent {
	this := UpdateTypeTemplatesResponseContent{}
	this.TypeName = typeName
	this.TypeSchema = typeSchema
	this.Description = description
	this.ChangeReason = changeReason
	this.CreatedBy = createdBy
	this.CreatedAt = createdAt
	this.LastModifiedAt = lastModifiedAt
	this.LastModifiedBy = lastModifiedBy
	return &this
}

// NewUpdateTypeTemplatesResponseContentWithDefaults instantiates a new UpdateTypeTemplatesResponseContent object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateTypeTemplatesResponseContentWithDefaults() *UpdateTypeTemplatesResponseContent {
	this := UpdateTypeTemplatesResponseContent{}
	return &this
}

// GetTypeName returns the TypeName field value
func (o *UpdateTypeTemplatesResponseContent) GetTypeName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TypeName
}

// GetTypeNameOk returns a tuple with the TypeName field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetTypeNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TypeName, true
}

// SetTypeName sets field value
func (o *UpdateTypeTemplatesResponseContent) SetTypeName(v string) {
	o.TypeName = v
}

// GetTypeSchema returns the TypeSchema field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *UpdateTypeTemplatesResponseContent) GetTypeSchema() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.TypeSchema
}

// GetTypeSchemaOk returns a tuple with the TypeSchema field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *UpdateTypeTemplatesResponseContent) GetTypeSchemaOk() (*interface{}, bool) {
	if o == nil || IsNil(o.TypeSchema) {
		return nil, false
	}
	return &o.TypeSchema, true
}

// SetTypeSchema sets field value
func (o *UpdateTypeTemplatesResponseContent) SetTypeSchema(v interface{}) {
	o.TypeSchema = v
}

// GetDescription returns the Description field value
func (o *UpdateTypeTemplatesResponseContent) GetDescription() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Description
}

// GetDescriptionOk returns a tuple with the Description field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Description, true
}

// SetDescription sets field value
func (o *UpdateTypeTemplatesResponseContent) SetDescription(v string) {
	o.Description = v
}

// GetChangeReason returns the ChangeReason field value
func (o *UpdateTypeTemplatesResponseContent) GetChangeReason() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ChangeReason
}

// GetChangeReasonOk returns a tuple with the ChangeReason field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetChangeReasonOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ChangeReason, true
}

// SetChangeReason sets field value
func (o *UpdateTypeTemplatesResponseContent) SetChangeReason(v string) {
	o.ChangeReason = v
}

// GetCreatedBy returns the CreatedBy field value
func (o *UpdateTypeTemplatesResponseContent) GetCreatedBy() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.CreatedBy
}

// GetCreatedByOk returns a tuple with the CreatedBy field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetCreatedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedBy, true
}

// SetCreatedBy sets field value
func (o *UpdateTypeTemplatesResponseContent) SetCreatedBy(v string) {
	o.CreatedBy = v
}

// GetCreatedAt returns the CreatedAt field value
func (o *UpdateTypeTemplatesResponseContent) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *UpdateTypeTemplatesResponseContent) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

// GetLastModifiedAt returns the LastModifiedAt field value
func (o *UpdateTypeTemplatesResponseContent) GetLastModifiedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.LastModifiedAt
}

// GetLastModifiedAtOk returns a tuple with the LastModifiedAt field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetLastModifiedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LastModifiedAt, true
}

// SetLastModifiedAt sets field value
func (o *UpdateTypeTemplatesResponseContent) SetLastModifiedAt(v time.Time) {
	o.LastModifiedAt = v
}

// GetLastModifiedBy returns the LastModifiedBy field value
func (o *UpdateTypeTemplatesResponseContent) GetLastModifiedBy() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.LastModifiedBy
}

// GetLastModifiedByOk returns a tuple with the LastModifiedBy field value
// and a boolean to check if the value has been set.
func (o *UpdateTypeTemplatesResponseContent) GetLastModifiedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LastModifiedBy, true
}

// SetLastModifiedBy sets field value
func (o *UpdateTypeTemplatesResponseContent) SetLastModifiedBy(v string) {
	o.LastModifiedBy = v
}

func (o UpdateTypeTemplatesResponseContent) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UpdateTypeTemplatesResponseContent) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["type_name"] = o.TypeName
	if o.TypeSchema != nil {
		toSerialize["type_schema"] = o.TypeSchema
	}
	toSerialize["description"] = o.Description
	toSerialize["change_reason"] = o.ChangeReason
	toSerialize["created_by"] = o.CreatedBy
	toSerialize["created_at"] = o.CreatedAt
	toSerialize["last_modified_at"] = o.LastModifiedAt
	toSerialize["last_modified_by"] = o.LastModifiedBy
	return toSerialize, nil
}

func (o *UpdateTypeTemplatesResponseContent) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type_name",
		"type_schema",
		"description",
		"change_reason",
		"created_by",
		"created_at",
		"last_modified_at",
		"last_modified_by",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varUpdateTypeTemplatesResponseContent := _UpdateTypeTemplatesResponseContent{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varUpdateTypeTemplatesResponseContent)

	if err != nil {
		return err
	}

	*o = UpdateTypeTemplatesResponseContent(varUpdateTypeTemplatesResponseContent)

	return err
}

type NullableUpdateTypeTemplatesResponseContent struct {
	value *UpdateTypeTemplatesResponseContent
	isSet bool
}

func (v NullableUpdateTypeTemplatesResponseContent) Get() *UpdateTypeTemplatesResponseContent {
	return v.value
}

func (v *NullableUpdateTypeTemplatesResponseContent) Set(val *UpdateTypeTemplatesResponseContent) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateTypeTemplatesResponseContent) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateTypeTemplatesResponseContent) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateTypeTemplatesResponseContent(val *UpdateTypeTemplatesResponseContent) *NullableUpdateTypeTemplatesResponseContent {
	return &NullableUpdateTypeTemplatesResponseContent{value: val, isSet: true}
}

func (v NullableUpdateTypeTemplatesResponseContent) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateTypeTemplatesResponseContent) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


