...
- **Tasks 4–11 (custom `MetricsMiddleware` + `HttpMeters`) replaced.** The hand-written Actix middleware was swapped for the upstream `opentelemetry-instrumentation-actix-web` 0.24 `RequestMetrics` middleware, configured with a custom `RouteFormatter` (preserves `__not_found__` / `__static__` cardinality sentinels) and a custom `metric_attrs_from_req` (normalizes HTTP method, conditionally attaches `sp.org_id` / `sp.workspace_id` per a process-wide `LabelConfig` installed via `OnceLock`). Bumped the OTel family to `0.32` and `prometheus` to `0.14`. The crate's body-size histograms are suppressed via an SDK View (`Aggregation::Drop`). The `http.server.busy.duration` Counter is gone — `rate(http_server_request_duration_seconds_sum[...])` is the equivalent saturation signal. Tasks 4 (module skeleton), 5–11 (meters/middleware build-out) — only the `MetricsMiddleware` / `HttpMeters` pieces are obsolete; the `ObservabilityConfig`, route extraction principles, and middleware-wiring intent carry over as design rationale.
