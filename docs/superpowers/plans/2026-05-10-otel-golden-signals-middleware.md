...
> **Status:** Plan executed; shipped with significant deviations. **Do not treat the task bodies below as ground truth** — they are preserved as historical record of original intent. The actual implementation is described by the (post-implementation-edited) design spec; this prelude lists which tasks were dropped or rewritten so a reader can navigate the task bodies appropriately.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

## Post-implementation deviations

The PR shipped with the following changes versus this plan. The canonical as-built design is in `docs/superpowers/specs/2026-05-10-otel-golden-signals-middleware-design.md`; this section maps tasks to outcomes.

### Tasks dropped or replaced wholesale

- **Tasks 4–11 (custom `MetricsMiddleware` + `HttpMeters`):** **replaced.** The hand-written Actix middleware (~470 lines, emitting `http.server.request.duration`, `http.server.busy.duration`, `http.server.active_requests` with custom `InFlightGuard`, label extraction, etc.) was swapped wholesale for the upstream `opentelemetry-instrumentation-actix-web 0.24` `RequestMetrics` middleware. The project-specific cardinality story (`__not_found__` / `__static__` route sentinels, HTTP method `_OTHER` normalization, optional `sp.org_id` / `sp.workspace_id` labels) is preserved via the crate's `RouteFormatter` and `with_metric_attrs_from_req` hooks; `LabelConfig` is installed via a `OnceLock` (`set_label_config()`) because the attribute hook is a function pointer with no closure state. The new code lives in `crates/service_utils/src/observability/instrumentation.rs` (~170 lines). Task 4's `meters.rs` and `middleware.rs` stub files were deleted.
- **Task 2 (`.cargo/config.toml` for `tokio_unstable`):** **dropped.** Tokio 1.50 exposes `Handle::metrics().num_workers()`, `.global_queue_depth()`, and `.worker_total_busy_duration(i)` as stable APIs.
...
- **Task 16 (Tokio runtime saturation):** **rewritten.** No background sampler, no `RuntimeMonitor`, no atomics snapshot, no `tokio-metrics` dep. Each observable callback reads `Handle::metrics()` directly. `runtime.tokio.workers.busy_ratio` (Gauge) is replaced with `runtime.tokio.workers.busy.time` (monotonic Counter, seconds, summed across workers); Prometheus computes saturation at query time. `runtime.tokio.tasks.alive` was dropped — `Handle::metrics()` doesn't expose it stably.
- **Task 21 (README + makefile `tokio_unstable` note):** **dropped** — no build flag required.

### Tasks edited (substance changed, structural intent preserved)

- **Task 1 / 3 (dependencies):** versions bumped — OTel family `0.27` → `0.32`, `prometheus` `0.13` → `0.14`. `opentelemetry-semantic-conventions` and `tokio-metrics` removed (attribute names inlined; runtime APIs stable). `opentelemetry-instrumentation-actix-web 0.24` added. `crates/superposition/Cargo.toml` does **not** add a direct `opentelemetry` dep — `service_utils::observability` re-exports `SdkMeterProvider`.
- **Task 14 (DB pool saturation):** `DbPoolHandle` is the bare `diesel::r2d2::Pool<...>` rather than `Arc<Pool<...>>` — r2d2's `Pool` is already internally `Arc`-cloneable, so the outer wrap was redundant.
- **Task 18 (`main.rs` wiring):** `SaturationDeps` no longer has `tokio_collect_interval`; the `.configure(configure_health_endpoints)` line is not present. The middleware is wrapped via `Condition::new(obs_enabled, metrics_middleware.clone())` and binds to an explicit `SdkMeterProvider` (not the OTel global) so parallel tests stay isolated.
...
**Architecture (as built):** New `service_utils::observability` module owns: (a) `Observability::init()` that builds an OTel `SdkMeterProvider` with a Prometheus exporter (+ optional OTLP HTTP push exporter) and an SDK View that drops body-size histograms, (b) a `build_request_metrics_middleware()` factory wrapping the upstream `opentelemetry-instrumentation-actix-web` `RequestMetrics` middleware with project-specific `RouteFormatter` + `metric_attrs_from_req` hooks, (c) saturation collectors for r2d2 DB pool, fred Redis pool, and Tokio runtime (all callback-driven, no background tasks), and (d) a separate `HttpServer` on `SUPERPOSITION_METRICS_PORT` exposing `/metrics`.

**Tech Stack (as built):** Rust, Actix-web 4, OpenTelemetry SDK 0.32 (`opentelemetry`, `opentelemetry_sdk`, `opentelemetry-prometheus`, `opentelemetry-otlp`, `opentelemetry-instrumentation-actix-web` 0.24), `prometheus` 0.14, `humantime` 2.1, Diesel/r2d2, fred (Redis client).
