...
Given(
  ...
  async function (this: SuperpositionWorld, name: string, value: string) {
...
Given(
  ...
  async function (this: SuperpositionWorld, name: string, value: string, desc: string) {
    this.variableName = name;
    try {
      await this.client.send(
        new CreateVariableCommand({
...
Given(
  ...
  async function (this: SuperpositionWorld, funcName: string, varName: string) {
...
When(
  ...
  async function (this: SuperpositionWorld, name: string, desc: string) {
    try {
      this.lastResponse = await this.client.send(
        new UpdateVariableCommand({
          workspace_id: this.workspaceId,
...
When(
  ...
  async function (this: SuperpositionWorld, funcName: string) {
    try {
      this.lastResponse = await this.client.send(
        new TestCommand({
          workspace_id: this.workspaceId,
...
Then(
  ...
  function (this: SuperpositionWorld, value: string) {
    assert.ok(this.lastResponse, "No response");
...
Then(
  ...
  async function (this: SuperpositionWorld, name: string, errorPattern: string) {
    try {
      await this.client.send(
        new GetVariableCommand({
          workspace_id: this.workspaceId,
