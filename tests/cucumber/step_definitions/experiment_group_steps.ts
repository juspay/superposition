...
When(
  ...
  async function (this: SuperpositionWorld, desc: string) {
    try {
      this.lastResponse = await this.client.send(
        new UpdateExperimentGroupCommand({
          workspace_id: this.workspaceId,
...
When(
  ...
  async function (this: SuperpositionWorld, id: string, traffic: number) {
    try {
      this.lastResponse = await this.client.send(
        new UpdateExperimentGroupCommand({
          workspace_id: this.workspaceId,
...
When("I list experiment groups", async function (this: SuperpositionWorld) {
  try {
    this.lastResponse = await this.client.send(
...
When(
  ...
  async function (this: SuperpositionWorld, sortOn: string, sortBy: string) {
    try {
      this.lastResponse = await this.client.send(
        new ListExperimentGroupsCommand({
          workspace_id: this.workspaceId,
...
Then(
  ...
  function (this: SuperpositionWorld, expected: number) {
    assert.ok(this.lastResponse, "No response");
