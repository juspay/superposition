use fred::prelude::{KeysInterface, RedisClient, RedisPool};
use serde::{de::DeserializeOwned, Serialize};
use superposition_macros::unexpected_error;
use superposition_types::result as superposition;

use crate::{db::PgSchemaConnectionPool, service::types::SchemaName};
    ...
    match get_data_from_redis(key.clone(), client).await {
        ...
        Err(e) => {
            ...
            if let Ok(ref value) = data {
                ...
                let _ = client
                    .set::<(), String, String>(key, serialized, None, None, false)
                    .await
                    .map_err(|e| {
