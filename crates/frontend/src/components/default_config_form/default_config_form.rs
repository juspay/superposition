    ...
    NF: Fn() + 'static + Clone,
{
    ...
    let handle_select_dropdown_option_enum = move |selected_enum: String| {
        set_config_value.update(|value| {
            let selected_enum_clone = selected_enum.clone();
            *value = selected_enum_clone;
        });
    };
                ...
                view! {
                    ...
                    <Show when=move || {
                        show_labels.get() && (config_type.get() != "number")
                            && (config_type.get() != "decimal") && (config_type.get() != "enum")
                    }>
                        <div class="form-control">
                            <label class="label">
                                <span class="label-text ">Value</span>
                    ...
                    <Show when=move || {
                        config_type.get() == "enum"
                    }>

                        {move || {
                            let enum_str = config_pattern.get();
                        ...
                        <div class="divider"></div>
                        {move || {
                            let enum_arr: Vec<String> = serde_json::from_str(&config_pattern.get())
                                .unwrap_or(vec![]);
                            let config_val = config_value.get();
                            let selected_enum = if config_val.len() == 0
                                || !enum_arr.contains(&config_val)
                            {
                            ...
                            view! {
                                <div class="form-control">
                                    <label class="label">
                                        <span class="label-text ">Choose Enum</span>
                                    </label>
                                    <div class="mt-2">
                                        <Dropdown
                                            ...
                                            dropdown_options=enum_arr
                                            on_select=Box::new(handle_select_dropdown_option_enum)
                                        />
                                    </div>
                                </div>
                            }
