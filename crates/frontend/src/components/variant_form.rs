    ...
    HC: Fn(Vec<(String, VariantFormT)>) + 'static + Clone,
{
    ...
    let combined_resource = create_blocking_resource(
        ...
        |(context, context_data, default_config, tenant, org_id)| async move {
            let (context_id, overrides) = match context_data {
                Some((id, overrides)) => (id, overrides),
    ...
    let update_control_variant =
        move |curr_variants: &mut Vec<(String, VariantFormT)>| {
            ...
            let overrides = combined_resource.get().unwrap_or_default().overrides;
    ...
    view! {
        <Suspense fallback=move || view! { <Skeleton /> }>
            <div class="form-control w-full">
                ...
                {move || {
                    let default_config = combined_resource.get().unwrap_or_default().default_config;
                    let default_config = StoredValue::new(default_config);
                    view! {
                        <For
                            each=move || {
