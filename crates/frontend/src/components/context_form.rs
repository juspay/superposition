...
use crate::types::{Dimension, ContextCondition};
...
pub fn context_form(
    ...
    context: ReadSignal<Vec<ContextCondition>>,
    set_context: WriteSignal<Vec<ContextCondition>>,
...
//     NF: Fn(Vec<(String, String, String)>) + 'static,
{
    ...
    let (used_dimensions, set_used_dimensions) = create_signal(
        context.get()
            .iter()
            .map(|ele| ele.dimension.get().clone())
    ...
    let handle_select_dropdown_option =
        Callback::new(move |selected_dimension: Dimension| {
            ...
            set_context.update(|value| {
                value.push(ContextCondition::new(dimension_name.clone(), "".to_string(), "".to_string()))
            });
                ...
                <div class="card w-full bg-slate-50">
                    <div class="card-body">
                        ...
                        {move || {
                            ...
                            view!{
                                <For
                                    each=move || {context.get().into_iter().enumerate().collect::<Vec<(usize, ContextCondition)>>()}
                                    key=|(idx, ele)| format!("{}-{}", ele.dimension.get(), idx)
                                    children=move |(idx, ele)| {
                                            let (dimension, operator, value) = (ele.dimension, ele.operator, ele.value);
                                            let dimension_label = dimension.get().to_string();
                                            let dimension_name = StoredValue::new(dimension.get().to_string());
                                            ...
                                            if operator.get().is_empty() {
                                                // set_context
                                                //     .update(|curr_context| {
                                                //         curr_context[idx].1 = String::from("==");
                                                //     });
                                                operator.set(String::from("==")); // ISSUE: because the component is not re rendering even updating the context in the above line.
                                            }
                                            view! {
                                                <div class="flex gap-x-6">
                                                    ...
                                                    <div class="form-control w-20">
                                                        ...
                                                        >
                                                            <option
                                                                value="=="
                                                                selected=operator.get() == "==" || resolve_mode
                                                            >
                                                                "IS"
                                                            </option>
                                                            <option value="in" selected=operator.get() == "in">
                                                                "HAS"
                                                            </option>
                                                            <option value="<=" selected=operator.get() == "<=">
                                                                "BETWEEN (inclusive)"
                                                            </option>
                                                        </select>
                                                                        ...
                                                                        match dimension_type.as_str() {
                                                                            ...
                                                                            "BOOLEAN" => {
                                                                                if value.get().is_empty() {
                                                                                    ...
                                                                                    //     });
                                                                                    value.update(|val| *val = val.parse::<bool>().unwrap_or(false).to_string());
                                                                                }
                                                                                view! {
                                                                                    <BooleanToggle
                                                                                        config_value=value.get()
