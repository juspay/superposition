    ...
    NF: Fn(Conditions) + 'static,
{
    ...
    let (context_rs, context_ws) = create_signal({
        ...
        dimensions
            ...
            .for_each(|dimension_info| {
                ...
                context
                    .borrow_mut()
                    .push(Condition::new_with_default_expression(
                ...
                update_context_with_dependent_dimensions(
                    ...
                    &|d_name, r#type| {
                        context
                            .borrow_mut()
                            .push(Condition::new_with_default_expression(d_name, r#type));
                    },
                );
            });
        context.into_inner()
    ...
    let get_tool_tip_text = move |condition: StoredValue<Condition>| {
        if mandatory_dimensions
            .get_value()
            .contains(&condition.get_value().variable)
        {
            "Mandatory Dimension".to_string()
        } else if dependent_dimension_locks_rs
            .get()
            .contains(&condition.get_value().variable)
        {
            dimension_map
                .get_value()
                .get(&condition.get_value().variable)
                .map(|d| d.immediate_parents.clone())
                .unwrap_or_default()
                .map(|parents| {
                    format!(
                        "Dependent on: {}",
