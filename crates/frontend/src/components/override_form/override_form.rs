                            ...
                            <div class="flex gap-x-6">
                                <div class="form-control w-20">
                                    <select class="select select-bordered">
                                        <option disabled selected>Select Config</option>
                                        <For
                                            each=move || configs.clone()
                                            key=|item: &DefaultConfig| item.key.to_string()
                                            children=move |item: DefaultConfig| {
                                                    view! {
                                                        <option value={item.key.to_string()} selected={item.key.to_string() == config_key_copy}>
                                                            {item.key}
                                                        </option>
                                                    }
                                            }
                                ...
                                <div class="form-control">
                                    <div class="flex gap-x-6 items-center">
                                        <input type="text" placeholder="Type here" value={config_value.to_string()} class="input input-bordered w-full max-w-xs" />
                                        <button
