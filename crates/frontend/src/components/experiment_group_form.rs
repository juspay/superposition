...
use crate::{
    api::experiment_group::{
        add_members_to_group, create_experiment_group, update_experiment_group,
    },
    components::{
        alert::AlertType,
        button::Button,
...
pub fn add_experiment_to_group_form(
    ...
    let change_reason_rws = create_rw_signal(ChangeReason::default());
        ...
        spawn_local({
            async move {
                let payload = ExpGroupMemberRequest {
                    change_reason: change_reason_rws.get_untracked(),
                    member_experiment_ids: members_selected_rws.get_untracked(),
                };

                let result = add_members_to_group(
                ...
                match result {
                    ...
                    Err(e) => {
                        logging::error!(
                            "Error submitting adding experiment group form: {}",
                            e
                        );
        ...
        <EditorProvider>
            <ChangeForm
                ...
                on_change=Callback::new(move |new_change_reason| {
                    let new_reason = match ChangeReason::try_from(new_change_reason) {
                        Ok(reason) => reason,
                        Err(err) => {
                            logging::error!("Failed to parse change reason: {}", err);
                            enqueue_alert(
                                format!("Failed to parse change reason: {}", err),
                                AlertType::Error,
                                5000,
                            );
                            return;
                    ...
                    change_reason_rws.set_untracked(new_reason)
                })
            />
...
pub fn experiment_group_form(
    ...
    let group_description_rws =
        create_rw_signal(Description::try_from(group_description).unwrap_or_default());
    let change_reason_rws = create_rw_signal(ChangeReason::default());
            ...
            async move {
                let result = if is_edit {
                    ...
                    let update_request = ExpGroupUpdateRequest {
                        change_reason: change_reason_rws.get(),
                        description: Some(group_description_rws.get()),
                        traffic_percentage: Some(traffic_percentage_rws.get()),
                    };
                    update_experiment_group(&group_id, update_request, &tenant, &org_id)
                        .await
                } else {
                    ...
                    let create_request = ExpGroupCreateRequest {
                        ...
                        description: group_description_rws.get(),
                        change_reason: change_reason_rws.get(),
                        context,
                        traffic_percentage: traffic_percentage_rws.get(),
                        member_experiment_ids: members,
                    };
                    create_experiment_group(create_request, &tenant, &org_id).await
                };
            ...
            <div>
                ...
                <ChangeForm
                    ...
                    on_change=Callback::new(move |new_description: String| {
                        let new_description = match Description::try_from(new_description) {
                            Ok(des) => des,
                            Err(err) => {
                                logging::error!("Failed to parse description: {}", err);
                                enqueue_alert(
                                    format!("Failed to parse description: {}", err),
                                    AlertType::Error,
                                    5000,
                                );
                                return;
                ...
                <div class="form-control w-full">
                    ...
                    <input
                        ...
                        on:input=move |ev| {
                            ...
                            let value = match TrafficPercentage::try_from(value) {
                                Ok(v) => v,
                                Err(err) => {
                                    logging::error!("Failed to parse traffic percentage: {}", err);
                                    enqueue_alert(
                                        format!("Failed to parse traffic percentage: {}", err),
                                        AlertType::Error,
                                        5000,
                                    );
                                    return;
