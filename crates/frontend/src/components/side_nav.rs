...
fn create_org_routes(org: &str) -> Vec<AppRoute> {
    ...
    vec![
        ...
        // TODO: Show only if AuthZ is enabled
        AppRoute {
            key: format!("{base}/admin/{org}/settings/authz"),
            path: format!("{base}/admin/{org}/settings/authz"),
            icon: "ri-lock-2-line".to_string(),
            label: "AuthZ".to_string(),
        },
    ]
...
pub fn OrgSideNav() -> impl IntoView {
    ...
    view! {
        <OrgNavComponent
            ...
            app_routes
        />
        <OrgNavComponent is_placeholder=false collapsed_rws app_routes />
    }
}
