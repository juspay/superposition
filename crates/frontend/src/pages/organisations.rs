...
use crate::types::{GenerateMasterKeyResponse, RotateMasterKeyResponse};
...
pub fn organisations() -> impl IntoView {
    ...
    let (_rotation_result, set_rotation_result) =
        create_signal::<Option<RotateMasterKeyResponse>>(None);
    let (is_generating, set_is_generating) = create_signal(false);
    let (is_rotating, set_is_rotating) = create_signal(false);
    let (show_master_key_rotate_modal, set_show_master_key_rotate_modal) =
        create_signal(false);
    let (show_master_key_generate_modal, set_show_master_key_generate_modal) =
        create_signal(false);
    let (master_key_change_reason, set_master_key_change_reason) =
    ...
    let rotate_master_key_action = create_action(move |_: &()| async move {
        set_is_rotating.set(true);
        set_rotation_result.set(None);

        let result =
            master_key::rotate(master_key_change_reason.get_untracked(), None).await;

        set_is_rotating.set(false);
        set_show_master_key_rotate_modal.set(false);

        match result {
            Ok(response) => {
                set_rotation_result.set(Some(response));
                set_master_key_change_reason.set(String::new());
            }
            Err(e) => {
    ...
    view! {
                    ...
                    <form class="h-full flex flex-col gap-4">
                        ...
                        <div class="flex gap-2 mb-2 ml-auto">
                            ...
                            <button
                                type="button"
                                class="btn btn-purple"
                                on:click=move |_| set_show_master_key_rotate_modal.set(true)
                            >
                                "Rotate Master Key"
        ...
        // Master Key Rotate Modal
        {move || {
            show_master_key_rotate_modal
                .get()
                .then(|| {
                    view! {
                        <PortalModal
                            heading="Rotate Master Encryption Key"
                            handle_close=Callback::new(move |_| {
                                set_show_master_key_rotate_modal.set(false);
                                set_master_key_change_reason.set(String::new());
                            })
                        >
                            <div class="alert alert-warning mb-4">
                                <i class="ri-alert-line"></i>
                                <span>
                                    "This will re-encrypt encryption keys for all workspaces across all organisations. This is a global operation."
                                </span>
                            </div>

                            <div class="form-control mb-4">
                                <label class="label">
                                    <span class="label-text">"Reason for rotation *"</span>
                                </label>
                                <textarea
                                    class="textarea textarea-bordered h-24"
                                    placeholder="e.g., Scheduled rotation, security policy..."
                                    on:input=move |ev| {
                                        set_master_key_change_reason.set(event_target_value(&ev))
                                    }
                                    prop:value=master_key_change_reason
                                />
                            </div>
                            ...
                            <div class="flex justify-end gap-3">
                                <Button
                                    ...
                                    icon_class="ri-close-line"
                                    force_style="btn px-5 py-2.5 rounded-lg"
                                />
                                <button
                                    class="btn btn-error px-5 py-2.5 rounded-lg"
                                    disabled=is_rotating.get()
                                        || master_key_change_reason.get().trim().is_empty()
                                    on:click=move |_| {
                                        if !master_key_change_reason.get().trim().is_empty() {
                                            rotate_master_key_action.dispatch(())
                                        }
                                    }
                                >
                                    {if is_rotating.get() { "Rotating..." } else { "Rotate Key" }}
                                    <i class="ri-refresh-line"></i>
                                </button>
                            </div>
                        </PortalModal>
