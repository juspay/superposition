...
use crate::{api::fetch_grouped_default_config, components::button::Button};
...
pub fn DefaultConfigList() -> impl IntoView {
    ...
    let default_config_resource = create_blocking_resource(
        ...
        |(workspace, pagination_params, org_id, filters)| async move {
            fetch_grouped_default_config(&pagination_params, filters, &workspace, &org_id)
                .await
                .unwrap_or_default()
        },
    );
    ...
    let get_create_button = move |loading| {
        view! {
            <Button
                ...
                <div class="flex items-end gap-4 lg+:flex-row flex-col-reverse">
                    ...
                    <div class="flex gap-4">
                        <DefaultConfigFilterWidget
                            filters_rws
                            pagination_params_rws
                            prefix=filters_rws.with(|p| p.prefix.clone())
                        />
                        <Suspense fallback=move || get_create_button(
                            true,
                        )>
                            {move || get_create_button(
                                default_config_resource.try_get().flatten().is_none(),
                            )}
                        </Suspense>
                    </div>
                </div>
