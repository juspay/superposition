...
pub fn Dimensions() -> impl IntoView {
    ...
    let selected_dimensions_rws = create_rw_signal::<HashSet<String>>(HashSet::new());
    let copy_all_rws = create_rw_signal(true);
    let show_copy_panel_rws = create_rw_signal(false);
    ...
    let selected_segment_class = "btn join-item min-h-12 flex-1 !border-purple-300 !bg-purple-50 !text-purple-800 hover:!border-purple-400 hover:!bg-purple-100";
    let idle_segment_class = "btn join-item min-h-12 flex-1 !border-slate-200 !bg-white !text-slate-700 hover:!border-slate-300 hover:!bg-slate-50";

    let table_columns = create_memo(move |_| {
        let selected_dimensions_rws = selected_dimensions_rws;
        let copy_all_rws = copy_all_rws;
        let show_copy_panel_rws = show_copy_panel_rws;
        vec![
            Column::new(
                "dimension".to_string(),
                false,
                    ...
                    view! {
                        <div class="flex items-center gap-3">
                            <input
                                ...
                                class:hidden=move || !show_copy_panel_rws.get()
                                prop:checked=move || {
                                ...
                                prop:disabled=move || {
                                    !show_copy_panel_rws.get() || copy_all_rws.get()
                                }
                                on:change=move |event| {
                                    let checked = event_target_checked(&event);
                                    selected_dimensions_rws.update(|selected| {
                                        if checked {
                                            selected.insert(changed_dimension_name.clone());
                                        } else {
                                            selected.remove(&changed_dimension_name);
                                        }
                                    });
                                }
                            />
                ...
                Expandable::Disabled,
                |_| {
                    view! { <span class="font-medium text-sm text-black">"Dimension"</span> }.into_view()
                },
            ),
    ...
    let on_copy = Callback::new(move |_| {
        ...
        let selected_dimensions = selected_dimensions_rws.get_untracked();
        if matches!(selection_mode, RowSelectionMode::Selected)
            && selected_dimensions.is_empty()
        {
            enqueue_alert(
        ...
        let mut selected_rows = selected_dimensions.into_iter().collect::<Vec<_>>();
                ...
                view! {
                        <div class="h-full flex flex-col gap-4">
                            ...
                            <div class="flex items-end gap-2">
                                <button
                                    class="btn btn-sm btn-outline h-10"
                                    type="button"
                                    on:click=move |_| {
                                        show_copy_panel_rws.update(|show| *show = !*show);
                                    }
                                >
                                    <i class="ri-file-copy-line"></i>
                                    {move || {
                                        if show_copy_panel_rws.get() {
                                            "Hide copy options"
                                        } else {
                                            "Copy to workspace"
                                        }
                        ...
                        <Show when=move || show_copy_panel_rws.get()>
                            <div class="card w-full bg-base-100 rounded-xl shadow">
                                <div class="card-body gap-4">
                                    ...
                                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                        <label class="form-control w-full">
                                            ...
                                            <select
                                                ...
                                                prop:value=move || target_workspace_rws.get()
                                                on:change=move |event| {
                                    ...
                                    <div class="flex justify-end">
                                        <button
                                            class="btn-purple px-5 py-2.5 rounded-lg"
                                            prop:disabled=move || {
