...
use superposition_types::{
    ...
    database::{
        models::experimentation::{ExperimentGroup, TrafficPercentage},
...
use crate::{
    api::{
        experiment_group::{delete_experiment_group, fetch_experiment_groups},
        fetch_dimensions,
    },
    components::{
        alert::AlertType,
    ...
    vec![
        Column::new(
            ...
            move |value: &str, row: &Map<String, Value>| {
                ...
                let group_id = row.get("id").map_or(String::from(""), |value| {
            ...
            move |_, row: &Map<String, Value>| {
                let group = RowData {
                    ...
                    traffic_percentage: row
                        ...
                        .and_then(|p| p.as_u64().map(|p| p.to_string()))
                        .and_then(|p| TrafficPercentage::try_from(p).ok())
                        .unwrap_or_default(),
                };
    ...
    let experiment_groups_resource = create_blocking_resource(
        ...
        |(filters, pagination, tenant, org_id)| async move {
            let experiment_groups =
                fetch_experiment_groups(&filters, &pagination, &tenant, &org_id)
                    .await
                    .unwrap_or_default();
            let dimensions =
                ...
                <DeleteModal
                    ...
                    confirm_delete=Callback::new(move |_| {
                        ...
                        spawn_local(async move {
                            ...
                            if let Err(e) = delete_experiment_group(&group_id, &tenant, &org_id)
                                .await
                            {
