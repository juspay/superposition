...
pub fn use_signal_from_query<T: TupleSignals + 'static>(
    ...
    let signals_for_effect2 = signals.clone();
    ...
    Effect::new(move |_| {
        ...
        if current_query != last_query {
            last_query_rws.set(current_query.clone());
            let new_value = parser(&current_query);
            T::update_signals(&signals_for_update_effect, new_value);
            return;
        }

        let desired_data = T::get_data(&signals_for_effect2);

        let desired_query = T::to_query_param(&signals_for_effect2);
        ...
        let current_data = T::destruct(parser2(&current_query));
