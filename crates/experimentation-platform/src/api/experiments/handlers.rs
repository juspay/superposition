...
use diesel::{QueryResult, RunQueryDsl, sql_query};
...
fn are_overlapping_contexts(context_a_json: &Value, context_b_json: &Value) -> bool {
    ...
    let ref_keys =
        if context_a_keys.len() > context_b_keys.len() {
            context_b_keys
        } else {
            context_a_keys
        };
    ...
    for key in ref_keys {
        let test =
            (context_a.contains_key(key) && context_b.contains_key(key))
...
#[post("")]
async fn create(state: Data<AppState>, req: web::Json<ExperimentCreateReq>) -> HttpResponse {
    ...
    if !req.context.is_object() {
        return HttpResponse::BadRequest()
                .json(
                    "{\"message\": \"context should be a map of key value pairs\" }"
                )
    }
    ...
    if
        !flags.allow_same_keys_overlapping_ctx
        ...
        || !flags.allow_same_keys_non_overlapping_ctx
    {
        ...
        for active_experiment in active_experiments.iter() {
            ...
            let have_intersecting_key_set =
                active_experiment.override_keys.iter().any(|key| override_keys_set.contains(key));
            ...
            if !flags.allow_same_keys_overlapping_ctx {
                valid_experiment = valid_experiment && (are_overlapping && !have_intersecting_key_set);
            }
    ...
    if !valid_experiment {
        return HttpResponse::BadRequest()
            .json("invalid experiment config");
    }
    ...
    for mut variant in variants {
        ...
        let fields_to_add = HashMap::from([ ("variant".to_string(), variant_id.to_string()) ]);
