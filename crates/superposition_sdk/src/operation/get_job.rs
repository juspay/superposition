...
#[derive(Debug)]
    ...
    ::std::write!(output, "/jobs/{id}", id = id).expect("formatting should succeed");
...
impl GetJobError {
    ...
    /// Returns `true` if the error kind is `GetJobError::ResourceNotFound`.
    pub fn is_resource_not_found(&self) -> bool {
        matches!(self, Self::ResourceNotFound(_))
    }
    /// Returns `true` if the error kind is `GetJobError::InternalServerError`.
