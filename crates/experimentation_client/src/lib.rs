...
use superposition_types::{
    ...
    database::models::experimentation::{ExperimentGroup, ExperimentGroups, ExperimentStatusType, Variant, VariantType},
...
impl Client {
    ...
    pub async fn get_applicable_variant(
        ...
        for exp in experiments {
            if let Some(v) =
                self.decide_variant( exp, toss).await?
            {
                variants.push(v.id)
            }
        }
...
async fn get_experiment_groups(
    ...
    loop {
        ...
        let endpoint =
            format!("{hostname}/experiment-groups?{pagination_params}");
        ...
        for experiment_group in experiment_groups.into_iter() {
            curr_exp_group_store.insert(experiment_group.id.to_string(), experiment_group);
        }
