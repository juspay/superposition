...
impl fmt::Display for HeadersEnum {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            HeadersEnum::ConfigVersion => write!(f, "x-config-version"),
        }
    }
}
...
pub struct WebhookConfig {
    pub enabled: bool,
    pub configuration: Option<Webhook>,
}
        ...
        D: Deserializer<'de>,
    {
        ...
        match (helper.enabled, helper.configuration) {
            ...
            (false, Some(_)) => Err(serde::de::Error::custom(
                "Configuration must be None when enabled is false",
            )),
            (enabled, configuration) => Ok(WebhookConfig {
                enabled,
                configuration,
            }),
        }
