...
use diesel::{deserialize::{self, FromSql}, pg::PgValue, serialize::{self, Output, ToSql}};
#[cfg(feature = "diesel_derives")]
use diesel::{
    sql_types::{Json, Text, Array},
    AsChangeset, AsExpression, FromSqlRow, Insertable, QueryId, Queryable, Selectable, pg::Pg,
};
...
impl TryFrom<Vec<Bucket>> for Buckets {
    ...
    fn try_from(value: Vec<Bucket>) -> Result<Self, Self::Error> {
        if value.len() != 100 {
            return Err(format!("Buckets must contain exactly 100 elements, got {}", value.len()));
        }
        Ok(Buckets(value))
    }
}
...
impl FromSql<Array<Json>, Pg> for Buckets {
    fn from_sql(bytes: PgValue<'_>) -> deserialize::Result<Self> {
        let json_array: Vec<serde_json::Value> = FromSql::<Array<Json>, Pg>::from_sql(bytes)?;
...
// #[cfg(feature = "diesel_derives")]
// impl diesel::deserialize::FromSqlRow<Array<Json>, Pg> for Buckets {
//     fn build_from_row<'a>(row: &impl Row<'a, Pg>) -> deserialize::Result<Self> {
//         FromSql::<Array<Json>, Pg>::from_sql(row.get(0)?)
//     }
// }
...
impl ToSql<Array<Json>, Pg> for Buckets {
    fn to_sql<'b>(&'b self, out: &mut Output<'b, '_, Pg>) -> serialize::Result {
        let json_values: Result<Vec<serde_json::Value>, _> = self.0
            .iter()
            .map(|bucket| serde_json::to_value(bucket))
            .collect();
        
        let json_array = json_values?;
        <Vec<serde_json::Value> as ToSql<Array<Json>, Pg>>::to_sql(&json_array, &mut out.reborrow())
    }
}