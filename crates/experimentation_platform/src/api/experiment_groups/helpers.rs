...
pub fn validate_members(
    ...
    let mut traffic_percentage_sum =
        existing_member_contexts.iter().fold(0, |acc, x| acc + **x);
    ...
    let members = HashSet::from_iter(new_members.to_owned());
    ...
    let mut accepted_members = Vec::from_iter(accepted_members);
    let mut existing_members = Vec::from_iter(existing_members.to_owned());
    accepted_members.append(&mut existing_members);
    Ok(accepted_members)
}
