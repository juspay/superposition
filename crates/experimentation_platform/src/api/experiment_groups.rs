...
async fn add_members_to_group(
    ...
    let DbConnection(mut conn) = db_conn;
    let id = exp_group_id.into_inner();
    helpers::add_members(&id, req, &mut conn, &schema_name, user)
}

#[post("/{exp_group_id}/remove-members")]
async fn remove_members_to_group(
    ...
    let DbConnection(mut conn) = db_conn;
    let id = exp_group_id.into_inner();
    helpers::remove_members(&id, req, &mut conn, &schema_name, user)
}
