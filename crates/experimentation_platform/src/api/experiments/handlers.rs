...
async fn create(
    ...
    let new_experiment = Experiment {
        ...
        experiment_group_id: req.experiment_group_id.unwrap_or(None),
    };
    ...
    let mut inserted_experiments =
        conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
            ...
            if let Some(Some(experiment_group_id)) = &req.experiment_group_id {
                add_members(
                    experiment_group_id,
            ...
            let updated_experiment =
                diesel::update(experiments::experiments.find(experiment_id))
                    .set((
                        ...
                        experiments::change_reason.eq(change_reason.clone()),
                        ...
                        experiments::experiment_group_id
                            .eq(experiment_group_id
                                .unwrap_or(experiment.experiment_group_id)),
                    ))
                    .returning(Experiment::as_returning())
                    .schema_name(&workspace_request.schema_name)
