...
use crate::api::{
    experiment_groups::helpers::{
        ...
        remove_experiment_from_group, remove_members, update_bucket_allocation,
    },
    experiments::{
        helpers::{
...
use super::{
    ...
    helpers::{
        ...
        get_experiment, hash, validate_experiment, validate_override_keys,
    },
...
async fn get_applicable_variants(
    ...
    let filtered_experiment_groups = experiment_groups.into_iter().filter(|exp| {
        ...
        (context.is_empty()
            ...
            && *exp.traffic_percentage as i8 >= req_data.toss
    });

    let mut variants = Vec::new();
    for exp in filtered_experiment_groups {
        if let Some(v) = decide_variant(&exp, req_data.toss, &mut conn, &schema_name)
            .map_err(|e| {
