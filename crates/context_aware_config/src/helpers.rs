...
use crate::{
    api::{
        ...
        functions::helpers::get_validation_function,
    },
...
fn evaluate_remote_cohorts_dependency(
    ...
    while let Some((ref cohort_dimension, ref based_on)) = stack.pop() {
        if let Some(data) = dimensions.get(cohort_dimension) {
            ...
            let Some(ref value_compute_function_name_) = data.autocomplete_function_name
            else {
...
pub fn validate_change_reason(
    ...
    let change_reason_validation_function =
        get_validation_function(FunctionType::ChangeReasonValidation, conn, schema_name)?;
