...
async fn create_default_config(
    ...
    let version_id =
        conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
            ...
            let version_id = add_config_version(&state, tags, transaction_conn, &tenant)?;
            Ok(version_id)
        })?;
    #[cfg(feature = "high-performance-mode")]
...
async fn update_default_config(
    ...
    let version_id =
        conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
            ...
            let version_id =
                add_config_version(&state, tags.clone(), transaction_conn, &tenant)?;
        ...
        let resp =
            conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
                ...
                match deleted_row {
                    ...
                    Ok(_) => {
                        version_id =
                            add_config_version(&state, tags, transaction_conn, &tenant)?;
                        log::info!(
