...
pub(crate) fn validate_default_config_value(
    schema: &Value,
    value: &Value,
) -> superposition::Result<()> {
    let compiled = try_into_jsonschema(schema)
...
pub(crate) async fn validate_default_config_functions(
    workspace_context: &WorkspaceContext,
    conn: &mut DBConnection,
    key: &str,
    value: &Value,
...
pub(super) async fn validate_create_request(
    ...
    .await?;
    validate_default_config_value(&Value::from(&request.schema), &request.value)?;
...
pub(super) async fn validate_update_request(
    ...
    validate_default_config_value(&Value::from(schema), value)?;
