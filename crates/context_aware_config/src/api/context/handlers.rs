...
fn ensure_description(
...
) -> Result<Option<String>, superposition::AppError> {
    ...
    match existing_context {
        Some(ctx) => Ok(Some(ctx.description)),
        None => Err(superposition::AppError::NotFound(format!(
            "Description not provided and no existing context found",
        ))),
    }
}
            ...
            for action in reqs.into_inner().into_iter() {
                match action {
                    ContextAction::Put(put_req) => {
                        ...
                        all_descriptions.push(description.unwrap_or_default());
...
async fn priority_recompute(
    ...
    use superposition_types::cac::schema::contexts::dsl::{contexts, priority};
    let DbConnection(mut conn) = db_conn;
    ...
    let config_version_id =
        conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
            let insert = diesel::insert_into(contexts)
                ...
                .execute(transaction_conn);
            // Extract description and change_reason from the response
...
async fn weight_recompute(
    ...
    let config_version_id =
        conn.transaction::<_, superposition::AppError, _>(|transaction_conn| {
            ...
            let change_reason = contexts_new_weight.iter().map(|(_, _, change_reason, _)| change_reason.clone()).collect::<Vec<String>>().join(",");
