...
use service_utils::{
    encryption::{decrypt_workspace_key, encrypt_secret, require_master_key},
...
fn get_workspace_keys(
    ...
    let master_key = require_master_key(&app_state.master_key).map_err(|e| {
        ...
        bad_argument!("Master key not configured. Configure MASTER_ENCRYPTION_KEY to use secrets")
    })?;

    let decrypted_key =
        decrypt_workspace_key(&workspace.encryption_key, master_key).map_err(|e| {
