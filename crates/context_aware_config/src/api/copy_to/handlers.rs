...
use service_utils::{
    ...
    middlewares::auth_z::{Action as AuthZAction, AuthZDomain, AuthZHandler},
    service::types::{
...
use superposition_macros::{
    bad_argument, db_error, forbidden, not_found, unexpected_error,
};
use superposition_types::{
    DBConnection, InternalUserContext, Resource, User,
    api::{
        copy_to::{
...
async fn authorize_copy_for_target_workspace(
    auth_z_handler: &AuthZHandler,
    target_workspace_context: &WorkspaceContext,
    user: &User,
    ...
    let target_domain = AuthZDomain::new(target_workspace_context.schema_name.0.clone());

    for dimension in source_dimensions {
        let action = if existing_target_dimensions.contains(&dimension.dimension) {
            if skip_existing {
                continue;
            }
            CopyToAuthZActionUpdate::get()
        ...
        let is_allowed = auth_z_handler
            ...
            .map_err(|err| {
                unexpected_error!(
                    "Error checking target workspace authorization: {}",
                    err
                )
            })?;
        ...
        if !is_allowed {
            return Err(forbidden!(
                "You are not authorized to copy dimensions into the target workspace."
            ));
        }
    }
...
async fn copy_to_handler(
    ...
    .await?;
    authorize_copy_for_target_workspace(
        &auth_z_handler,
        &target_workspace_context,
        &user,
        &internal_user,
