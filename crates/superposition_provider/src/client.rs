...
impl CacConfig {
    ...
    async fn get_config_static(options: &SuperpositionOptions) -> Result<Config> {
        ...
        // Create SDK config
        let sdk_config = SdkConfig::builder().endpoint_url(&options.endpoint).bearer_token(options.token.clone().into()).build();
...
impl ExperimentationConfig {
    ...
    async fn get_experiments_static(
        ...
        // Create SDK config
        let sdk_config = SdkConfig::builder().endpoint_url(&options.endpoint).build();
