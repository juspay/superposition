...
impl SuperpositionMcpServer {
    pub async fn get_config_impl(
        ...
        if let Some(ctx) = args.context {
            req = req.set_context(Some(json_to_doc_map(ctx).map_err(mcp_err)?));
        }
    ...
    pub async fn resolve_config_impl(
        &self,
        args: ResolveConfigParams,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let mut req = self
            .client
            .get_resolved_config()
        ...
        if let Some(sr) = args.show_reasoning {
            req = req.show_reasoning(sr);
        }
        if let Some(ms) = args.merge_strategy {
            req = req.merge_strategy(if ms.to_uppercase() == "REPLACE" {
                superposition_sdk::types::MergeStrategy::Replace
            } else {
                superposition_sdk::types::MergeStrategy::Merge
            });
        }
        if let Some(cid) = args.context_id {
            req = req.context_id(cid);
        }
        if let Some(rr) = args.resolve_remote {
            req = req.resolve_remote(rr);
        }
        let resp = req.send().await.map_err(mcp_err)?;
    ...
    pub async fn resolve_config_with_identifier_impl(
        &self,
        args: ResolveConfigWithIdentifierParams,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let mut req = self
            .client
