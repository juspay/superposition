...
pub enum VariantSelection {
    Simple,       // Basic traffic threshold check
    Bucketed,     // Complex bucket distribution like existing client
    Experimental, // Prefer experimental variants for toss < 0
}
...
impl Default for EvaluationOptions {
    fn default() -> Self {
        Self {
            context_eval: ContextEvaluation::default(),
    ...
    for experiment in experiments {
        ...
        let context_matches = match opts.context_eval {
            ...
            ContextEvaluation::Partial => {
                match jsonlogic::partial_apply(
                    ...
                    | Ok(jsonlogic::PartialApplyOutcome::Ambiguous) => true,
                    _ => false,
                }
            }
        };
        ...
        if traffic_passes {
            ...
            let experiment_variants: Vec<&Variant> = variants
                .iter()
                .filter(|v| v.id.to_string() == experiment.id)
                .collect();
...
// Experimental variant selection: prefer experimental variants for toss < 0
fn select_variants_experimental<'a>(
    ...
    if toss < 0 {
        ...
        // For non-negative toss, return all applicable variants
        variants.to_vec()
    }
}
