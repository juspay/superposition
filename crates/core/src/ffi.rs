...
pub extern "C" fn core_evaluate_experiments(
    ...
    let options = if evaluation_opts.is_null() {
        None
    } else {
        match parse_json::<crate::experiment::EvaluationOptions>(evaluation_opts) {
            Ok(opts) => Some(opts),
            Err(e) => {
                set_last_error(format!("Failed to parse options: {}", e));
                return ptr::null_mut();
            }
        }
    };
    ...
    // Call experiment.rs function (like how config calls are made)
