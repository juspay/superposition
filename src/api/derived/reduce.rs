...
#[delete("")]
pub async fn reduce_contexts_overrides(
    ...
    let mapped_context_ids: Vec<String> = ctx_overrides
        .iter()
        .map(|i| i.context_id.to_owned())
        .collect();

    let mapped_override_ids: Vec<String> = ctx_overrides
        .iter()
        .map(|i| i.override_id.to_owned())
        .collect();

    let all_context_ids: Vec<String> = fetch_all_new_contexts(state)
        .await?
        .iter()
        .map(|x| x.key.to_owned())
        .collect();
    let all_override_ids: Vec<String> = get_all_overrides(state)
        .await?
        .iter()
        .map(|x| x.key.to_owned())
        .collect();
