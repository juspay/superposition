...
use crate::api::primary::{
    context_overrides::{
        add_ctx_override, delete_ctx_override_helper, fetch_override_from_ctx_id,
        ContextOverrideResponse,
    },
    new_contexts::{add_new_context_v2, delete_new_context_helper, process_input_context_json},
};
use crate::utils::{
    errors::{AppError, AppErrorType::SomethingWentWrong},
...
#[post("")]
pub async fn promote_contexts_overrides(
    input_state: Data<AppState>,
    body: Json<KeyValue>,
    ...
    let get_processed_context_and_hash_values =
        |keys_to_be_excluded: Option<&Vec<String>>| -> Result<(String, Value), AppError> {
    ...
    let (_, new_context_value) =
        get_processed_context_and_hash_values(Some(&Vec::from([dimension_to_be_dropped])))?;
