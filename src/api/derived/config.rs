...
async fn _get_context_overrides_object(state: &Data<AppState>, query_string: &str) -> Result<Value, AppError> {
    if query_string == "" {
...
async fn get_new_context_overrides_object (state: &Data<AppState>, query_string: &str) -> Result<Value, AppError> {
        ...
        match (item.get("id"), item.get("condition")) {
            (Some(context_id), Some(condition)) => {
                ...
                if let Ok(override_id) =
                    fetch_override_from_ctx_id(
                        &state,
                        strip_double_quotes(&context_id.to_string())
                    ).await {
