...
CREATE TABLE IF NOT EXISTS {replaceme}.response_templates (
...
CREATE INDEX IF NOT EXISTS idx_response_templates_name ON {replaceme}.response_templates(name);
CREATE INDEX IF NOT EXISTS idx_response_templates_created_at ON {replaceme}.response_templates(created_at);
CREATE INDEX IF NOT EXISTS idx_response_templates_last_modified_at ON {replaceme}.response_templates(last_modified_at);
CREATE TRIGGER response_templates_audit AFTER INSERT OR DELETE OR UPDATE ON {replaceme}.response_templates FOR EACH ROW EXECUTE FUNCTION public.event_logger();
