import {
    ...
    type GetExperimentGroupCommandOutput,
    type UpdateExperimentGroupCommandInput,
    type ListExperimentGroupsCommandOutput,
...
describe("Experiment Groups API Integration Tests", () => {
    ...
    const dimensionsToEnsure: { name: string; schema: any; description: string }[] = [
        { name: "os", schema: { type: "string", enum: ["ios", "android", "web"] }, description: "OS dimension from experiments.test" },
        { name: "clientId", schema: { type: "string" }, description: "Client ID from experiments.test" },
        { name: "app_version", schema: { type: "string" }, description: "App version for superset context" },
        { name: "device_specific_id", schema: { type: "string" }, description: "Dimension for invalid context conflict" },
    ];

    // Define default configs based on experiments.test.ts
    const defaultConfigsToEnsure: { key: string; value: any; schema: any; description: string }[] = [
        { key: "pmTestKey1", value: "default_group_val_1", schema: { type: "string" }, description: "Default for pmTestKey1 (group tests)" },
        { key: "pmTestKey2", value: "default_group_val_2", schema: { type: "string" }, description: "Default for pmTestKey2 (group tests)" },
    ];
    
    // Contexts using the ensured dimensions
    const groupContext = { "and": [{ "==": [{ "var": "os" }, "ios"] }, { "==": [{ "var": "clientId" }, "clientForExpGroup"] }] };
    const expValid1Context = { ...groupContext }; // Exact match
    const expValid2Context = { "and": [...(groupContext.and as any[]), { "==": [{ "var": "app_version" }, "2.0.0"] }] }; // Superset
    const expInvalidContextConflict = { "and": [{ "==": [{ "var": "device_specific_id" }, "devSpecificXYZ"] }] };
    ...
    const defaultVariantInputsForGroupExperiments: CreateExperimentCommandInput['variants'] = [
        {
            ...
            id: "group_control_A",
            overrides: { pmTestKey1: "group_control_A_val", pmTestKey2: "group_control_B_val" },
        },
        {
            variant_type: VariantType.EXPERIMENTAL,
            id: "group_experimental_A",
            overrides: { pmTestKey1: "group_experimental_A_val", pmTestKey2: "group_experimental_B_val" },
        },
    ];


    async function ensureDimensionExists(name: string, schema: any, description: string, existingDimensions: DimensionExt[]): Promise<void> {
        const existing = existingDimensions.find(d => d.dimension === name);
        if (!existing) {
            let position = 1;
            const takenPositions = new Set(existingDimensions.map(d => d.position));
            while(takenPositions.has(position)) {
                position++;
            }

            console.log(`Dimension ${name} not found, creating with position ${position}...`);
            await superpositionClient.send(new CreateDimensionCommand({
            ...
            existingDimensions.push({ dimension: name, position } as DimensionExt);
        } else {
            console.log(`Dimension ${name} already exists.`);
        }
    }

    async function ensureDefaultConfigExists(key: string, value: any, schema: any, description: string, existingConfigs: DefaultConfigFull[]): Promise<void> {
        const existing = existingConfigs.find(c => c.key === key);
        if (!existing) {
            console.log(`Default config ${key} not found, creating...`);
            await superpositionClient.send(new CreateDefaultConfigCommand({
                workspace_id: ENV.workspace_id,
                org_id: ENV.org_id,
                key: key,
                value: value,
    ...
    beforeAll(async () => {
        console.log("Ensuring dimensions and default configs exist for Experiment Group tests...");
        const listDimResponse = await superpositionClient.send(new ListDimensionsCommand({
            workspace_id: ENV.workspace_id,
            org_id: ENV.org_id,
            count: 200, 
        }));
        ...
        for (const dim of dimensionsToEnsure) {
            await ensureDimensionExists(dim.name, dim.schema, dim.description, existingDimensions);
        }

        const listConfigResponse = await superpositionClient.send(new ListDefaultConfigsCommand({
            workspace_id: ENV.workspace_id,
            org_id: ENV.org_id,
            count: 200,
        }));
        ...
        for (const cfg of defaultConfigsToEnsure) {
            await ensureDefaultConfigExists(cfg.key, cfg.value, cfg.schema, cfg.description, existingConfigs);
        }
        ...
        const exp1Response = await superpositionClient.send(new CreateExperimentCommand(createExp1Input));
        ...
        const exp2Response = await superpositionClient.send(new CreateExperimentCommand(createExp2Input));
        ...
        const createExp3Input: CreateExperimentCommandInput = { // Context for this one can be simple as status is the key
            workspace_id: ENV.workspace_id,
            org_id: ENV.org_id,
        ...
        const exp3Response = await superpositionClient.send(new CreateExperimentCommand(createExp3Input));
        ...
        console.log(`Created and ramped expInvalidInProgressId: ${expInvalidInProgressId}`);
        ...
        const exp4Response = await superpositionClient.send(new CreateExperimentCommand(createExp4Input));
    ...
    afterAll(async () => {
        ...
        for (const id of createdExperimentIds) {
            try {
                ...
                console.log(`Discarded experiment: ${id}`);
                 ...
                 if (error.name !== "ResourceNotFound" && error.name !== "ExperimentNotFound") {
                    console.error(`Failed to discard experiment ${id} during cleanup:`, error.message);
                }
            }
        }
        ...
        for (const dimName of createdDimensionNames) {
            try {
                await superpositionClient.send(new DeleteDimensionCommand({
                    workspace_id: ENV.workspace_id,
                    org_id: ENV.org_id,
                    dimension: dimName,
                }));
                ...
                if (error.name !== "ResourceNotFound") {
                    console.error(`Failed to clean up dimension ${dimName}:`, error.message);
                }
            }
        }
        for (const configKey of createdDefaultConfigKeys) {
            try {
                await superpositionClient.send(new DeleteDefaultConfigCommand({
                    workspace_id: ENV.workspace_id,
                    org_id: ENV.org_id,
                    key: configKey,
                }));
                ...
                if (error.name !== "ResourceNotFound") {
                    console.error(`Failed to clean up default config ${configKey}:`, error.message);
                }
            }
        }
    });
    ...
    describe("CreateExperimentGroupCommand", () => {
        const baseCreateInput: Omit<CreateExperimentGroupCommandInput, "name" | "member_experiment_ids"> = {
            workspace_id: ENV.workspace_id,
            org_id: ENV.org_id,
        ...
        test("should successfully create an experiment group with valid members", async () => {
            ...
            try {
                const response = await superpositionClient.send(new CreateExperimentGroupCommand(input));
                console.log("Response on create", response);
                expGroupId = response.experiment_group_id!;
                expect(response.name).toBe(groupName);
                expect(response.member_experiment_ids).toEqual(expect.arrayContaining([expValid1Id, expValid2Id]));
        ...
        test("should fail to create with an in-progress experiment member", async () => {
            ...
            expect(
                superpositionClient.send(new CreateExperimentGroupCommand(input))
            ).rejects.toThrow("Some experiment Ids do not exist/are invalid");
        });
        ...
        test("should fail to create with a conflicting context experiment member", async () => {
            ...
            expect(
                superpositionClient.send(new CreateExperimentGroupCommand(input))
            ).rejects.toThrow(/Experiment with id .* does not fit in with the experiment group. The contexts do not match./);
        });

        test("should fail if name is missing", async () => {
            const input = { ...baseCreateInput, member_experiment_ids: [] } as any;
            delete input.name;
            expect(
                superpositionClient.send(new CreateExperimentGroupCommand(input))
            ).rejects.toThrow(); 
        ...
        test("should fail if traffic_percentage is invalid (e.g., > 100)", async () => {
            ...
            expect(
                superpositionClient.send(new CreateExperimentGroupCommand(input))
            ).rejects.toThrow("JSON Parse error: Unexpected identifier \"Json\"\n  Deserialization error: to see the raw response, inspect the hidden field {error}.$response on this object."); 
        });
    });
    ...
    describe("GetExperimentGroupCommand", () => {
        test("should successfully get an existing experiment group", async () => {
            ...
            const response = await superpositionClient.send(
                new GetExperimentGroupCommand({
                    workspace_id: ENV.workspace_id,
                    org_id: ENV.org_id,
                    experiment_group_id: expGroupId!,
                })
            );
        ...
        test("should fail to get a non-existent experiment group", async () => {
            ...
            ).rejects.toThrow("No records found. Please refine or correct your search parameters");
        });
    });
    ...
    describe("UpdateExperimentGroupCommand", () => {
        test("should successfully update traffic_percentage", async () => {
            ...
            const currentGroup = await superpositionClient.send(new GetExperimentGroupCommand({workspace_id: ENV.workspace_id, org_id: ENV.org_id, experiment_group_id: expGroupId!}));
            ...
            const input: UpdateExperimentGroupCommandInput = {
                ...
                member_experiment_ids: currentGroup.member_experiment_ids || [], 
            };
            const response = await superpositionClient.send(new UpdateExperimentGroupCommand(input));
        ...
        test("should successfully update member_experiment_ids", async () => {
            expect(expGroupId).toBeString();
            const input: UpdateExperimentGroupCommandInput = {
                workspace_id: ENV.workspace_id,
                org_id: ENV.org_id,
                ...
                change_reason: "Updating members",
            };
            const response = await superpositionClient.send(new UpdateExperimentGroupCommand(input));
            expect(response.member_experiment_ids).toEqual([expValid1Id]);
        });
        
        test("should successfully clear member_experiment_ids", async () => {
            expect(expGroupId).toBeString();
            const input: UpdateExperimentGroupCommandInput = {
                workspace_id: ENV.workspace_id,
                org_id: ENV.org_id,
                experiment_group_id: expGroupId!,
                member_experiment_ids: [], 
                change_reason: "Clearing members",
            };
            const response = await superpositionClient.send(new UpdateExperimentGroupCommand(input));
            expect(response.member_experiment_ids).toEqual([]);
        });

        test("should fail to update with an invalid member experiment", async () => {
            expect(expGroupId).toBeString();
            const input: UpdateExperimentGroupCommandInput = {
                workspace_id: ENV.workspace_id,
                org_id: ENV.org_id,
            ...
            expect(
                superpositionClient.send(new UpdateExperimentGroupCommand(input))
            ).rejects.toThrow("Some experiment Ids do not exist/are invalid");
        });
    ...
    describe("ListExperimentGroupsCommand", () => {
        test("should list experiment groups and find the created one", async () => {
            expect(expGroupId).toBeString();
            const groupDetails = await superpositionClient.send(new GetExperimentGroupCommand({workspace_id: ENV.workspace_id, org_id: ENV.org_id, experiment_group_id: expGroupId!}));
            ...
            const found = response.data!.find(g => g.experiment_group_id === expGroupId);
    ...
    describe("DeleteExperimentGroupCommand", () => {
        ...
        beforeAll(async () => { 
            ...
            const response = await superpositionClient.send(new CreateExperimentGroupCommand(createInput));
            tempGroupId = response.experiment_group_id!;
        });
            ...
            expect(
                superpositionClient.send(
                    new DeleteExperimentGroupCommand({
                        workspace_id: ENV.workspace_id,
                        org_id: ENV.org_id,
                        experiment_group_id: tempGroupId,
                    })
                )
            ).rejects.toThrow(/Cannot delete experiment group .* since it has members/);
        });
