module Io.Superposition.Model.SecretSortOn (
    SecretSortOn(..)
) where
...
-- Enum implementation for SecretSortOn
data SecretSortOn =
    NAME
    | CREATED_AT
    | LAST_MODIFIED_AT
...
instance Data.Aeson.ToJSON SecretSortOn where
...
instance Data.Aeson.FromJSON SecretSortOn where
    parseJSON = Data.Aeson.withText "SecretSortOn" $ \v ->
        case v of
            "name" -> pure NAME
            "created_at" -> pure CREATED_AT
            "last_modified_at" -> pure LAST_MODIFIED_AT
            _ -> fail $ "Unknown value for SecretSortOn: " <> Data.Text.unpack v
        
    

instance Io.Superposition.Utility.SerDe SecretSortOn where
    ...
    deSerializeElement bs = case Data.Text.Encoding.decodeUtf8 bs of
        ...
        e -> Left ("Failed to de-serialize SecretSortOn, encountered unknown variant: " ++ (show bs))
