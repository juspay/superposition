...
newtype OrganisationResponseBuilder a = OrganisationResponseBuilder {
    runOrganisationResponseBuilder :: OrganisationResponseBuilderState -> (OrganisationResponseBuilderState, a)
}

instance Data.Functor.Functor OrganisationResponseBuilder where
    fmap f (OrganisationResponseBuilder g) =
        OrganisationResponseBuilder (\s -> let (s', a) = g s in (s', f a))

instance Control.Applicative.Applicative OrganisationResponseBuilder where
    pure a = OrganisationResponseBuilder (\s -> (s, a))
    (OrganisationResponseBuilder f) <*> (OrganisationResponseBuilder g) = OrganisationResponseBuilder (\s ->
        let (s', h) = f s
            (s'', a) = g s'
        in (s'', h a))

instance Control.Monad.Monad OrganisationResponseBuilder where
    (OrganisationResponseBuilder f) >>= g = OrganisationResponseBuilder (\s ->
        let (s', a) = f s
            (OrganisationResponseBuilder h) = g a
        in h s')

setId' :: Data.Text.Text -> OrganisationResponseBuilder ()
setId' value =
   OrganisationResponseBuilder (\s -> (s { id'BuilderState = Data.Maybe.Just value }, ()))

setName :: Data.Text.Text -> OrganisationResponseBuilder ()
setName value =
   OrganisationResponseBuilder (\s -> (s { nameBuilderState = Data.Maybe.Just value }, ()))

setCountryCode :: Data.Maybe.Maybe Data.Text.Text -> OrganisationResponseBuilder ()
setCountryCode value =
   OrganisationResponseBuilder (\s -> (s { country_codeBuilderState = value }, ()))

setContactEmail :: Data.Maybe.Maybe Data.Text.Text -> OrganisationResponseBuilder ()
setContactEmail value =
   OrganisationResponseBuilder (\s -> (s { contact_emailBuilderState = value }, ()))

setContactPhone :: Data.Maybe.Maybe Data.Text.Text -> OrganisationResponseBuilder ()
setContactPhone value =
   OrganisationResponseBuilder (\s -> (s { contact_phoneBuilderState = value }, ()))

setCreatedBy :: Data.Text.Text -> OrganisationResponseBuilder ()
setCreatedBy value =
   OrganisationResponseBuilder (\s -> (s { created_byBuilderState = Data.Maybe.Just value }, ()))

setAdminEmail :: Data.Text.Text -> OrganisationResponseBuilder ()
setAdminEmail value =
   OrganisationResponseBuilder (\s -> (s { admin_emailBuilderState = Data.Maybe.Just value }, ()))

setStatus :: Io.Superposition.Model.OrgStatus.OrgStatus -> OrganisationResponseBuilder ()
setStatus value =
   OrganisationResponseBuilder (\s -> (s { statusBuilderState = Data.Maybe.Just value }, ()))

setSector :: Data.Maybe.Maybe Data.Text.Text -> OrganisationResponseBuilder ()
setSector value =
   OrganisationResponseBuilder (\s -> (s { sectorBuilderState = value }, ()))

setCreatedAt :: Data.Time.UTCTime -> OrganisationResponseBuilder ()
setCreatedAt value =
   OrganisationResponseBuilder (\s -> (s { created_atBuilderState = Data.Maybe.Just value }, ()))

setUpdatedAt :: Data.Time.UTCTime -> OrganisationResponseBuilder ()
setUpdatedAt value =
   OrganisationResponseBuilder (\s -> (s { updated_atBuilderState = Data.Maybe.Just value }, ()))

setUpdatedBy :: Data.Text.Text -> OrganisationResponseBuilder ()
setUpdatedBy value =
   OrganisationResponseBuilder (\s -> (s { updated_byBuilderState = Data.Maybe.Just value }, ()))

build :: OrganisationResponseBuilder () -> Data.Either.Either Data.Text.Text OrganisationResponse
build builder = do
    let (st, _) = runOrganisationResponseBuilder builder defaultBuilderState
