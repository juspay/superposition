...
setStrictMode :: Bool -> GetWorkspaceOutputBuilder ()
setStrictMode value =
   Control.Monad.State.Strict.modify (\s -> (s { strict_modeBuilderState = Data.Maybe.Just value }))

setMetrics :: Data.Maybe.Maybe Data.Aeson.Value -> GetWorkspaceOutputBuilder ()
setMetrics value =
...
build builder = do
    ...
    strict_mode' <- Data.Maybe.maybe (Data.Either.Left "Io.Superposition.Model.GetWorkspaceOutput.GetWorkspaceOutput.strict_mode is a required property.") Data.Either.Right (strict_modeBuilderState st)
