module Io.Superposition.Model.ListSecretsInput (
    setCount,
    setPage,
    setAll',
    setWorkspaceId,
...
import qualified Io.Superposition.Model.SecretSortOn
import qualified Io.Superposition.Model.SortBy
import qualified Io.Superposition.Utility
import qualified Network.HTTP.Types.Method

data ListSecretsInput = ListSecretsInput {
...
instance Data.Aeson.ToJSON ListSecretsInput where
...
instance Io.Superposition.Utility.SerializeBody ListSecretsInput

instance Data.Aeson.FromJSON ListSecretsInput where
    parseJSON = Data.Aeson.withObject "ListSecretsInput" $ \v -> ListSecretsInput
...
data ListSecretsInputBuilderState = ListSecretsInputBuilderState {
...
defaultBuilderState :: ListSecretsInputBuilderState
defaultBuilderState = ListSecretsInputBuilderState {
...
type ListSecretsInputBuilder = Control.Monad.State.Strict.State ListSecretsInputBuilderState

setCount :: Data.Maybe.Maybe Data.Int.Int32 -> ListSecretsInputBuilder ()
setCount value =
   Control.Monad.State.Strict.modify (\s -> (s { countBuilderState = value }))

setPage :: Data.Maybe.Maybe Data.Int.Int32 -> ListSecretsInputBuilder ()
setPage value =
   Control.Monad.State.Strict.modify (\s -> (s { pageBuilderState = value }))

setAll' :: Data.Maybe.Maybe Bool -> ListSecretsInputBuilder ()
setAll' value =
   Control.Monad.State.Strict.modify (\s -> (s { all'BuilderState = value }))

setWorkspaceId :: Data.Text.Text -> ListSecretsInputBuilder ()
setWorkspaceId value =
   Control.Monad.State.Strict.modify (\s -> (s { workspace_idBuilderState = Data.Maybe.Just value }))

setOrgId :: Data.Text.Text -> ListSecretsInputBuilder ()
setOrgId value =
...
setSortOn :: Data.Maybe.Maybe Io.Superposition.Model.SecretSortOn.SecretSortOn -> ListSecretsInputBuilder ()
setSortOn value =
   Control.Monad.State.Strict.modify (\s -> (s { sort_onBuilderState = value }))

setSortBy :: Data.Maybe.Maybe Io.Superposition.Model.SortBy.SortBy -> ListSecretsInputBuilder ()
setSortBy value =
   Control.Monad.State.Strict.modify (\s -> (s { sort_byBuilderState = value }))

build :: ListSecretsInputBuilder () -> Data.Either.Either Data.Text.Text ListSecretsInput
build builder = do
    ...
    workspace_id' <- Data.Maybe.maybe (Data.Either.Left "Io.Superposition.Model.ListSecretsInput.ListSecretsInput.workspace_id is a required property.") Data.Either.Right (workspace_idBuilderState st)
    org_id' <- Data.Maybe.maybe (Data.Either.Left "Io.Superposition.Model.ListSecretsInput.ListSecretsInput.org_id is a required property.") Data.Either.Right (org_idBuilderState st)
    ...
    Data.Either.Right (ListSecretsInput { 
        count = count',
        page = page',
        all' = all'',
...
instance Io.Superposition.Utility.IntoRequestBuilder ListSecretsInput where
