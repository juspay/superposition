...
newtype WorkspaceResponseBuilder a = WorkspaceResponseBuilder {
    runWorkspaceResponseBuilder :: WorkspaceResponseBuilderState -> (WorkspaceResponseBuilderState, a)
}

instance Data.Functor.Functor WorkspaceResponseBuilder where
    fmap f (WorkspaceResponseBuilder g) =
        WorkspaceResponseBuilder (\s -> let (s', a) = g s in (s', f a))

instance Control.Applicative.Applicative WorkspaceResponseBuilder where
    pure a = WorkspaceResponseBuilder (\s -> (s, a))
    (WorkspaceResponseBuilder f) <*> (WorkspaceResponseBuilder g) = WorkspaceResponseBuilder (\s ->
        let (s', h) = f s
            (s'', a) = g s'
        in (s'', h a))

instance Control.Monad.Monad WorkspaceResponseBuilder where
    (WorkspaceResponseBuilder f) >>= g = WorkspaceResponseBuilder (\s ->
        let (s', a) = f s
            (WorkspaceResponseBuilder h) = g a
        in h s')

setWorkspaceName :: Data.Text.Text -> WorkspaceResponseBuilder ()
setWorkspaceName value =
   WorkspaceResponseBuilder (\s -> (s { workspace_nameBuilderState = Data.Maybe.Just value }, ()))

setOrganisationId :: Data.Text.Text -> WorkspaceResponseBuilder ()
setOrganisationId value =
   WorkspaceResponseBuilder (\s -> (s { organisation_idBuilderState = Data.Maybe.Just value }, ()))

setOrganisationName :: Data.Text.Text -> WorkspaceResponseBuilder ()
setOrganisationName value =
   WorkspaceResponseBuilder (\s -> (s { organisation_nameBuilderState = Data.Maybe.Just value }, ()))

setWorkspaceSchemaName :: Data.Text.Text -> WorkspaceResponseBuilder ()
setWorkspaceSchemaName value =
   WorkspaceResponseBuilder (\s -> (s { workspace_schema_nameBuilderState = Data.Maybe.Just value }, ()))
...
setWorkspaceStatus value =
   WorkspaceResponseBuilder (\s -> (s { workspace_statusBuilderState = Data.Maybe.Just value }, ()))

setWorkspaceAdminEmail :: Data.Text.Text -> WorkspaceResponseBuilder ()
setWorkspaceAdminEmail value =
   WorkspaceResponseBuilder (\s -> (s { workspace_admin_emailBuilderState = Data.Maybe.Just value }, ()))

setConfigVersion :: Data.Maybe.Maybe Data.Text.Text -> WorkspaceResponseBuilder ()
setConfigVersion value =
   WorkspaceResponseBuilder (\s -> (s { config_versionBuilderState = value }, ()))

setCreatedBy :: Data.Text.Text -> WorkspaceResponseBuilder ()
setCreatedBy value =
   WorkspaceResponseBuilder (\s -> (s { created_byBuilderState = Data.Maybe.Just value }, ()))

setLastModifiedBy :: Data.Text.Text -> WorkspaceResponseBuilder ()
setLastModifiedBy value =
   WorkspaceResponseBuilder (\s -> (s { last_modified_byBuilderState = Data.Maybe.Just value }, ()))

setLastModifiedAt :: Data.Time.UTCTime -> WorkspaceResponseBuilder ()
setLastModifiedAt value =
   WorkspaceResponseBuilder (\s -> (s { last_modified_atBuilderState = Data.Maybe.Just value }, ()))

setCreatedAt :: Data.Time.UTCTime -> WorkspaceResponseBuilder ()
setCreatedAt value =
   WorkspaceResponseBuilder (\s -> (s { created_atBuilderState = Data.Maybe.Just value }, ()))
...
setAutoPopulateControl value =
   WorkspaceResponseBuilder (\s -> (s { auto_populate_controlBuilderState = Data.Maybe.Just value }, ()))
