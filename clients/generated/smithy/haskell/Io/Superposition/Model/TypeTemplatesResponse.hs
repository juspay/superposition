...
newtype TypeTemplatesResponseBuilder a = TypeTemplatesResponseBuilder {
    runTypeTemplatesResponseBuilder :: TypeTemplatesResponseBuilderState -> (TypeTemplatesResponseBuilderState, a)
}

instance Data.Functor.Functor TypeTemplatesResponseBuilder where
    fmap f (TypeTemplatesResponseBuilder g) =
        TypeTemplatesResponseBuilder (\s -> let (s', a) = g s in (s', f a))

instance Control.Applicative.Applicative TypeTemplatesResponseBuilder where
    pure a = TypeTemplatesResponseBuilder (\s -> (s, a))
    (TypeTemplatesResponseBuilder f) <*> (TypeTemplatesResponseBuilder g) = TypeTemplatesResponseBuilder (\s ->
        let (s', h) = f s
            (s'', a) = g s'
        in (s'', h a))

instance Control.Monad.Monad TypeTemplatesResponseBuilder where
    (TypeTemplatesResponseBuilder f) >>= g = TypeTemplatesResponseBuilder (\s ->
        let (s', a) = f s
            (TypeTemplatesResponseBuilder h) = g a
        in h s')

setTypeName :: Data.Text.Text -> TypeTemplatesResponseBuilder ()
setTypeName value =
   TypeTemplatesResponseBuilder (\s -> (s { type_nameBuilderState = Data.Maybe.Just value }, ()))

setTypeSchema :: Data.Aeson.Value -> TypeTemplatesResponseBuilder ()
setTypeSchema value =
   TypeTemplatesResponseBuilder (\s -> (s { type_schemaBuilderState = Data.Maybe.Just value }, ()))

setDescription :: Data.Text.Text -> TypeTemplatesResponseBuilder ()
setDescription value =
   TypeTemplatesResponseBuilder (\s -> (s { descriptionBuilderState = Data.Maybe.Just value }, ()))

setChangeReason :: Data.Text.Text -> TypeTemplatesResponseBuilder ()
setChangeReason value =
   TypeTemplatesResponseBuilder (\s -> (s { change_reasonBuilderState = Data.Maybe.Just value }, ()))

setCreatedBy :: Data.Text.Text -> TypeTemplatesResponseBuilder ()
setCreatedBy value =
   TypeTemplatesResponseBuilder (\s -> (s { created_byBuilderState = Data.Maybe.Just value }, ()))

setCreatedAt :: Data.Time.UTCTime -> TypeTemplatesResponseBuilder ()
setCreatedAt value =
   TypeTemplatesResponseBuilder (\s -> (s { created_atBuilderState = Data.Maybe.Just value }, ()))

setLastModifiedAt :: Data.Time.UTCTime -> TypeTemplatesResponseBuilder ()
setLastModifiedAt value =
   TypeTemplatesResponseBuilder (\s -> (s { last_modified_atBuilderState = Data.Maybe.Just value }, ()))

setLastModifiedBy :: Data.Text.Text -> TypeTemplatesResponseBuilder ()
setLastModifiedBy value =
   TypeTemplatesResponseBuilder (\s -> (s { last_modified_byBuilderState = Data.Maybe.Just value }, ()))

build :: TypeTemplatesResponseBuilder () -> Data.Either.Either Data.Text.Text TypeTemplatesResponse
build builder = do
    let (st, _) = runTypeTemplatesResponseBuilder builder defaultBuilderState
