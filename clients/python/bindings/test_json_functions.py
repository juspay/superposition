...
def test_parse_json_config():
    """Test the ffi_parse_json_config function"""
...
def test_with_external_file():
    """Test parsing a JSON file from the examples directory"""
    print_section_header("TEST 2: Parse External JSON File")
    ...
    print(f"\nReading JSON from: {example_file}")
...
def test_error_handling():
    ...
    invalid_json_cases = [
        {"name": "Invalid JSON syntax", "json": "{ invalid json content }"},
        {
    ...
    for i, case in enumerate(invalid_json_cases, 1):
        print(f"\nTest {i}: {case['name']}")
        print("-" * 50)
...
def test_toml_json_equivalence():
    """Test that TOML and JSON produce equivalent Config objects"""
    print_section_header("TEST 4: TOML/JSON Equivalence")
...
"""
    ...
    try:
        ...
        assert len(toml_result.contexts) == len(json_result.contexts), \
            f"contexts mismatch: {len(toml_result.contexts)} vs {len(json_result.contexts)}"
        assert len(toml_result.dimensions) == len(json_result.dimensions), \
            f"dimensions mismatch: {len(toml_result.dimensions)} vs {len(json_result.dimensions)}"
...
def main():
    """Run all tests"""
    print("\n" + "=" * 70)
    print("  SUPERPOSITION JSON PARSING - PYTHON BINDING TESTS")
    print("=" * 70)

    results = []

    # Run tests
    results.append(("Parse JSON", test_parse_json_config()))
    results.append(("External File", test_with_external_file()))

    # Error handling test (doesn't return pass/fail)
    test_error_handling()

    results.append(("TOML/JSON Equivalence", test_toml_json_equivalence()))
