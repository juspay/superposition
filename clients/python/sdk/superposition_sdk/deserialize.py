...
async def _deserialize_get_detailed_resolved_config(http_response: HTTPResponse, config: Config) -> GetDetailedResolvedConfigOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_get_detailed_resolved_config(http_response, config)
    ...
    return GetDetailedResolvedConfigOutput(**kwargs)

async def _deserialize_error_get_detailed_resolved_config(http_response: HTTPResponse, config: Config) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_get_experiment_config(http_response: HTTPResponse, config: Config) -> GetExperimentConfigOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_get_experiment_config(http_response, config)
    ...
    if body:
        codec = JSONCodec(default_timestamp_format=TimestampFormat.EPOCH_SECONDS)
        deserializer = codec.create_deserializer(body)
        body_kwargs = GetExperimentConfigOutput.deserialize_kwargs(deserializer)
        kwargs.update(body_kwargs)
    ...
    return GetExperimentConfigOutput(**kwargs)

async def _deserialize_error_get_experiment_config(http_response: HTTPResponse, config: Config) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_get_resolved_config_explanation(http_response: HTTPResponse, config: Config) -> GetResolvedConfigExplanationOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_get_resolved_config_explanation(http_response, config)
    ...
    if body:
        codec = JSONCodec(default_timestamp_format=TimestampFormat.EPOCH_SECONDS)
        kwargs["explanation"] = codec.deserialize(body, ResolveExplanation)
    ...
    return GetResolvedConfigExplanationOutput(**kwargs)

async def _deserialize_error_get_resolved_config_explanation(http_response: HTTPResponse, config: Config) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
