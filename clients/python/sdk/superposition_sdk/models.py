...
from ._private.schemas import (
    ...
    UPDATE_WORKSPACE_DATABASE as _SCHEMA_UPDATE_WORKSPACE_DATABASE,
    UPDATE_WORKSPACE_DATABASE_INPUT as _SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT,
    UPDATE_WORKSPACE_DATABASE_OUTPUT as _SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT,
...
@dataclass(kw_only=True)
class UpdateWorkspaceDatabaseInput:

    org_id: str = "juspay"
    workspace_name: str | None = None

    def serialize(self, serializer: ShapeSerializer):
        serializer.write_struct(_SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT, self)
    ...
    def deserialize_kwargs(cls, deserializer: ShapeDeserializer) -> dict[str, Any]:
        ...
        def _consumer(schema: Schema, de: ShapeDeserializer) -> None:
            match schema.expect_member_index():
                case 0:
                    kwargs["org_id"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT.members["org_id"])

                case 1:
                    kwargs["workspace_name"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT.members["workspace_name"])

                case _:
                    logger.debug("Unexpected member schema: %s", schema)

        deserializer.read_struct(_SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT, consumer=_consumer)
        return kwargs

@dataclass(kw_only=True)
class UpdateWorkspaceDatabaseOutput:
    ...
    def serialize(self, serializer: ShapeSerializer):
        serializer.write_struct(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT, self)

    def serialize_members(self, serializer: ShapeSerializer):
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_name"], self.workspace_name)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["organisation_id"], self.organisation_id)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["organisation_name"], self.organisation_name)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_schema_name"], self.workspace_schema_name)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_status"], self.workspace_status)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_admin_email"], self.workspace_admin_email)
        if self.config_version is not None:
            serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["config_version"], self.config_version)

        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["created_by"], self.created_by)
        serializer.write_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["last_modified_by"], self.last_modified_by)
        serializer.write_timestamp(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["last_modified_at"], self.last_modified_at)
        serializer.write_timestamp(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["created_at"], self.created_at)
        if self.mandatory_dimensions is not None:
            _serialize_list_mandatory_dimensions(serializer, _SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["mandatory_dimensions"], self.mandatory_dimensions)

        serializer.write_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["strict_mode"], self.strict_mode)
        if self.metrics is not None:
            serializer.write_document(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["metrics"], self.metrics)

        serializer.write_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["allow_experiment_self_approval"], self.allow_experiment_self_approval)
        serializer.write_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["auto_populate_control"], self.auto_populate_control)
    ...
    def deserialize_kwargs(cls, deserializer: ShapeDeserializer) -> dict[str, Any]:
        ...
        def _consumer(schema: Schema, de: ShapeDeserializer) -> None:
            match schema.expect_member_index():
                case 0:
                    kwargs["workspace_name"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_name"])

                case 1:
                    kwargs["organisation_id"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["organisation_id"])

                case 2:
                    kwargs["organisation_name"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["organisation_name"])

                case 3:
                    kwargs["workspace_schema_name"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_schema_name"])

                case 4:
                    kwargs["workspace_status"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_status"])

                case 5:
                    kwargs["workspace_admin_email"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["workspace_admin_email"])

                case 6:
                    kwargs["config_version"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["config_version"])

                case 7:
                    kwargs["created_by"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["created_by"])

                case 8:
                    kwargs["last_modified_by"] = de.read_string(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["last_modified_by"])

                case 9:
                    kwargs["last_modified_at"] = de.read_timestamp(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["last_modified_at"])

                case 10:
                    kwargs["created_at"] = de.read_timestamp(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["created_at"])

                case 11:
                    kwargs["mandatory_dimensions"] = _deserialize_list_mandatory_dimensions(de, _SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["mandatory_dimensions"])

                case 12:
                    kwargs["strict_mode"] = de.read_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["strict_mode"])

                case 13:
                    kwargs["metrics"] = de.read_document(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["metrics"])

                case 14:
                    kwargs["allow_experiment_self_approval"] = de.read_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["allow_experiment_self_approval"])

                case 15:
                    kwargs["auto_populate_control"] = de.read_boolean(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT.members["auto_populate_control"])

                case _:
                    logger.debug("Unexpected member schema: %s", schema)

        deserializer.read_struct(_SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT, consumer=_consumer)
        return kwargs

UPDATE_WORKSPACE_DATABASE = APIOperation(
        input = UpdateWorkspaceDatabaseInput,
        output = UpdateWorkspaceDatabaseOutput,
        schema = _SCHEMA_UPDATE_WORKSPACE_DATABASE,
        input_schema = _SCHEMA_UPDATE_WORKSPACE_DATABASE_INPUT,
        output_schema = _SCHEMA_UPDATE_WORKSPACE_DATABASE_OUTPUT,
        error_registry = TypeRegistry({
            ShapeID("io.superposition#InternalServerError"): InternalServerError,
        }),
        effective_auth_schemes = [
