...
//         console.log('  evaluationOptions:', evaluationOptions);

//         // Validate JSON strings are not empty or null
...
export class NativeResolver {
    ...
    resolveConfig(
        ...
        const overridesJson = JSON.stringify(overrides);
        const queryDataJson = JSON.stringify(queryData);
        // Pass null instead of empty array to avoid filtering everything
        ...
        // Call with ALL 6 parameters matching the Rust function signature
        const result = this.lib.core_get_resolved_config(
            defaultConfigsJson,
            contextsJson,
            overridesJson,
    ...
    resolveConfigWithReasoning(
        ...
        const result = this.lib.core_get_resolved_config_with_reasoning(
            ...
            JSON.stringify(queryData),
            mergeStrategy,
            filterPrefixesJson  // This will be null if no filtering needed
        );
