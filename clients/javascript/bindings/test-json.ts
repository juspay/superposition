...
function testParseJsonConfig(): boolean {
    ...
    try {
        ...
        const result = resolver.parseConfigFileWithFilters(
            EXAMPLE_JSON,
            "json",
        );
        ...
        Object.entries(overrides)
            ...
            .forEach(([id, data]) => {
                console.log(
                    `  ${id}: ${JSON.stringify(data).substring(0, 100)}...`,
                );
            });
...
function testWithExternalFile(): boolean | null {
    ...
    try {
        ...
        console.log(
            `  - Default config keys: ${Object.keys(result.default_configs).length}`,
        );
...
`;
        ...
        const tomlResult = resolver.parseConfigFileWithFilters(
            tomlConfig,
            "toml",
        );
        const jsonResult = resolver.parseConfigFileWithFilters(
            jsonConfig,
            "json",
        );
...
function main(): number {
    console.log("\n" + "=".repeat(70));
    console.log(
        "  SUPERPOSITION JSON PARSING - JAVASCRIPT/TYPESCRIPT BINDING TESTS",
    );
    console.log("=".repeat(70));
